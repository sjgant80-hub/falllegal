// audit.test.mjs — the layout must reproduce what falllegal has ALREADY written.
...
console.log('\n=== §1 · ⚑ THE HISTORICAL BYTES, UNCHANGED ===');
{
  ...
  const legacy = (prev, e) => prev + e.ts + e.action + e.adviserId + e.clientId + e.matterId + JSON.stringify(e.payload || {});
  const e = {"ts":1723550000003,"action":"matter.updated","adviserId":"a1","clientId":"c1","matterId":"m1","payload":{"id":"x"}};
  const prev = 'abc123';
...
console.log('\n=== §3 · it catches what an audit log exists to catch ===');
{
  ...
  let log = [];
  for (let i = 1; i <= 4; i++) {
    log = [...log, await chain.append(log, { id: 'a' + i, ts: 1000 + i, action: 'saved',
